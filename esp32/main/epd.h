...
class EInkDisplay {
 public:
    ...
    void refreshDisplay(RefreshMode mode = FAST_REFRESH, bool turnOffScreen = false) {
        ...
        ESP_LOGI("epd", "refreshDisplay: mode=%s displayMode=0x%02X",
                 mode == FULL_REFRESH ? "FULL" : mode == HALF_REFRESH ? "HALF" : "FAST",
                 displayMode);
    ...
    void setCustomLUT(bool enabled, const uint8_t* lutData = nullptr) {
        if (enabled && lutData) {
            ...
            for (uint16_t i = 0; i < 105; i++) sendData(lutData[i]);
            sendCommand(CMD_GATE_VOLTAGE);   sendData(lutData[105]);
            sendCommand(CMD_SOURCE_VOLTAGE); sendData(lutData[106]); sendData(lutData[107]); sendData(lutData[108]);
            sendCommand(CMD_WRITE_VCOM);     sendData(lutData[109]);
 ...
 private:
    ...
    void resetDisplay() {
        gpio_set_level(EPD_RST, 1); delay(20);
        gpio_set_level(EPD_RST, 0); delay(2);
        gpio_set_level(EPD_RST, 1); delay(20);
    }
    ...
    void initDisplayController() {
        ...
        sendData(0xAE); sendData(0xC7); sendData(0xC3); sendData(0xC0); sendData(0x40);
        ...
        sendCommand(CMD_AUTO_WRITE_BW_RAM);  sendData(0xF7); waitWhileBusy("CMD_AUTO_WRITE_BW_RAM");
        sendCommand(CMD_AUTO_WRITE_RED_RAM); sendData(0xF7); waitWhileBusy("CMD_AUTO_WRITE_RED_RAM");
    }
    ...
    void setRamArea(uint16_t x, uint16_t y, uint16_t w, uint16_t h) {
        ...
        sendData(x % 256);         sendData(x / 256);
        sendData((x+w-1) % 256);   sendData((x+w-1) / 256);

        sendCommand(CMD_SET_RAM_Y_RANGE);
        sendData((y+h-1) % 256);   sendData((y+h-1) / 256);
        sendData(y % 256);         sendData(y / 256);
