...
// ---------------------------------------------------------------------------
// Bulk comparison: auto-discover ALL .epub files in test/books/ and run the
// full layout comparison for each.
// ---------------------------------------------------------------------------
...
INSTANTIATE_TEST_SUITE_P(AllBooks, BulkMrbComparisonTest, ::testing::ValuesIn(get_all_test_books()),
...
INSTANTIATE_TEST_SUITE_P(AllBooks, StreamingMrbComparisonTest, ::testing::ValuesIn(get_all_test_books()),
                         [](const ::testing::TestParamInfo<std::string>& info) {
                           auto name = fs::path(info.param).stem().string();
                           for (auto& c : name) {
                             if (!std::isalnum(static_cast<unsigned char>(c)))
...
INSTANTIATE_TEST_SUITE_P(AllBooks, ConversionBenchmark, ::testing::ValuesIn(get_all_test_books()),
                         [](const ::testing::TestParamInfo<std::string>& info) {
                           auto name = fs::path(info.param).stem().string();
                           for (auto& c : name) {
                             if (!std::isalnum(static_cast<unsigned char>(c)))
