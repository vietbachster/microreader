...
- **Proportional font system** (`BitmapFont.h`, `BitmapFontFormat.h`, `DrawBuffer.h`):
  ...
  - **BitmapFontSet**: Wraps up to 5 `BitmapFont*` (Small/Normal/Large/XLarge/XXLarge, indexed by `FontSize` enum, `kFontSizeCount=5`). Implements `IFont`, dispatches by size, falls back to Normal. Also provides `glyph_data(ch, style, size)` for rendering.
  - **DrawBuffer::draw_text_proportional()**: Renders text using `BitmapFontSet` + `FontSize`.
  - **Font sizes**: Small=20px, Normal=24px, Large=26px, XLarge=28px, XXLarge=30px (configurable via `--bundle-sizes` in `generate_font.py`). CSS `font-size` maps to `FontSize` enum in `CssParser.h`. HTML `h1`→XXLarge, `h2`→XLarge, `h3`→Large, `small/sub/sup`→Small.
  ...
  - **ESP32 storage**: Fonts stored in spiffs partition (3.375MB), mmapped via `font_partition.h`. The firmware now includes a zlib-compressed copy of the FNTS bundle (`platforms/esp32/font_bundle.bin`, ~928KB) embedded via `EMBED_FILES`. On first book open (or after a firmware update), `FontPartition::provision_embedded()` stream-decompresses it into the partition (~15s one-time cost). CRC32 of the compressed bytes is stored in the partition header to skip re-provisioning on subsequent boots. Manual serial uploads still work but are overwritten on the next book open (the serial `write()` path stores CRC=0, triggering re-provisioning). After regenerating fonts with `generate_font.py`, run `python tools/compress_font_bundle.py` to update `font_bundle.bin` and check it in.
