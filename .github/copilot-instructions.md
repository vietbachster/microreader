...
- **Proportional font system** (`BitmapFont.h`, `BitmapFontFormat.h`, `DrawBuffer.h`):
  ...
  - **ESP32 storage**: Fonts live in the SPIFFS-style font partition (3.375 MB), mmapped via `font_partition.h`. The font bundles (zlib-compressed FNTS) are **not** part of the firmware image — they are appended to `firmware.bin` *after* the IDF image segments via the asset blob mechanism (see below), so they never consume DROM/MMU pages at boot. On first book open (or after a firmware update with a new font CRC), `FontPartition::provision_embedded()` stream-decompresses the requested bundle into the partition (~15s one-time cost). The partition header stores the manifest CRC32 to skip re-provisioning on subsequent boots. After regenerating fonts with `generate_font.py`, just rebuild — `tools/build_assets.py` is invoked from the PIO post-build hook and picks up the new `.bin` files automatically.
  - **Asset blob (appended to firmware.bin)**: `tools/build_assets.py` packages `bookerly.bin`, `alegreya.bin`, and the three sleep `.mgr` images into `assets.bin` (manifest = `[ASTS:4][version:4=1][count:4][total:4]` + N×44-byte entries `{name[32], offset, length, crc32}`). The PIO post-build hook `tools/copy_firmware.py` pads `firmware.bin` to a 4 KB boundary and appends the blob. ESP-IDF only memory-maps bytes inside image-header-declared segments, so the appended data is flashed (and OTA-updated) along with the firmware but never consumes MMU pages. Runtime access goes through `platforms/esp32/asset_blob.{h,cpp}` (`asset_blob::g_assets.init()` in `app_main`, then `map(name, &size, &handle)` / `unmap(handle)` / `crc(name)` using `esp_partition_mmap` on the running partition). `FontManager::ensure_ready()` and `DrawBuffer::show_sleep_image_embedded()` map their asset on demand and unmap immediately after use. **Why this exists**: putting fonts in DROM consumed ~32 of 64 MMU pages, leaving too few free pages for `esp_image_verify` to map the other partition's 3 MB segment 0 during OTA — `esp_ota_set_boot_partition` failed with `ESP_ERR_OTA_VALIDATE_FAILED`. The slim image needs only ~17 MMU pages, leaving ~47 free for OTA verify.
