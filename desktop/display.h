...
class DesktopEmulatorDisplay final : public microreader::IDisplay {
 public:
  ...
  void present(const microreader::DisplayFrame& frame, microreader::RefreshMode /*mode*/) override {
    ...
    for (int y = 0; y < microreader::DisplayFrame::kHeight; ++y) {
      uint8_t* row = p + y * pitch;
      for (int x = 0; x < microreader::DisplayFrame::kWidth; ++x) {
    ...
    const int win_w = sideways ? microreader::DisplayFrame::kHeight : microreader::DisplayFrame::kWidth;
    const int win_h = sideways ? microreader::DisplayFrame::kWidth : microreader::DisplayFrame::kHeight;

    // Position the (always 800×480) texture centred in the logical window so
    // SDL_RenderCopyEx rotates it about the correct pivot.
    SDL_Rect dst = {(win_w - microreader::DisplayFrame::kWidth) / 2, (win_h - microreader::DisplayFrame::kHeight) / 2,
